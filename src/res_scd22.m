...
%   Description: 
%        Obtain the probability of resolution of MVDR SCD estimators for different frequency separations.
%        
%
%   Notes: 
...
for idx = 1:length(snr)   % for each filter length
    for idx2 = 1:length(df)
        ...
        for trial=[1:trials]    % Repeat experiment trials times
            ...
            % SCD Estimation
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [S1 f1] = scd2(yn,M,'MVDR',fs,2*freqs(1));
            [S2 f2] = scd2(yn,M,'MVDR',fs,2*freqs(2));
            [S3 f3] = scd2(yn,M,'MVDR',fs,freqs(1)+freqs(2));
...
title('Probability of resolution of MVDR SCD Estimator','FontSize',16,'FontWeight', 'bold');
...
print -deps -painters  img/scd_res22

...
%     The correlation functions are obtained individually for each subcarrier and 
%     then averaged to reduce the noise effects.
...
for idxEbNo = 1:length(EbNo)
	...
	for trial = 1:trials
		...
		Sxx = zeros(Nfft, size(xcptime,2));
		Syy = zeros(Nfft, size(xcptime,2));
		Sxy = zeros(Nfft, size(xcptime,2));
		
		% For each subcarrier
		for carrier=1:size(ycptime,2)
			Sxx(:,carrier) = pwelch(xcptime(:,carrier),[],[],Nfft, Fs);
			Syy(:,carrier) = pwelch(ycptime(:,carrier),[],[],Nfft, Fs);
			Sxy(:,carrier) = cpsd(xcptime(:,carrier),ycptime(:,carrier),[],[], Nfft, Fs);
		end

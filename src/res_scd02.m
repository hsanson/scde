...
%   Description: 
%        Obtain the probability of resolution of MVDR SCD estimators for different SNR
...
for idx = 1:length(M)   % for each filter length
    for idx2 = 1:length(snr)
        ...
        for trial=[1:trials]    % Repeat experiment trials times
            ...
            % SCD Estimation
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [S1 f1] = scd2(yn,M(idx),'MVDR',fs,2*fc(1));
            [S2 f2] = scd2(yn,M(idx),'MVDR',fs,2*fc(2));
            [S3 f3] = scd2(yn,M(idx),'MVDR',fs,fc(1)+fc(2));
...
print -deps -painters  img/scd_res02

...
%     Includes example of preamble generation.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Preamble Generation (8.4.6.1.1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 128-FFT Preamble Modulation PN sequences (Table 309c)
cell_0_seg_0 = hex2bin('01E52A9B3');
cell_0_seg_1 = hex2bin('AA37BDA7C');
cell_0_seg_2 = hex2bin('3FE158D96');

% BPSK modulate the preamble PN sequence
preamble0 = 4*sqrt(2)*((1/2)*cell_0_seg_0);
preamble1 = 4*sqrt(2)*((1/2)*cell_0_seg_1);
preamble2 = 4*sqrt(2)*((1/2)*cell_0_seg_2);

% Build the preamble and convert to time domain
preamble = zeros(Nfft,1);
for(i=1:length(preamble0))
    preamble(11 + ((i-1)*3)) = preamble0(i);
end

for(i=1:length(preamble1))
    preamble(12 + ((i-1)*3)) = preamble1(i);
end

for(i=1:length(preamble2))
    preamble(13 + ((i-1)*3)) = preamble2(i);
end

% Convert to time domain
preamble = ifft(preamble);

% Add CP to the preamble symbol
cp_size = Nfft*G;
tail_start = Nfft - cp_size + 1;
tail_end = Nfft;

tmp_syms = [preamble(tail_start:tail_end,:); preamble];  % Add tail
preamble = tmp_syms(:)';
...
for idx=start_search:end_search
    time_corr_long(1,idx-start_search+1) = sum(xcorr(yrf(1,idx:idx+Nfft),preamble));
end
...
ysynced = yrf(:,fine_time_est+1:length(yrf(1,:)));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Return to Freq Domain
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
data_sig_len = length(ysynced);
cp_size = Nfft*G;
n_data_syms = floor(data_sig_len/(cp_size+Nfft));

% Cut to multiples of symbol period (Ts)
ytime = ysynced(1:n_data_syms*(cp_size+Nfft));

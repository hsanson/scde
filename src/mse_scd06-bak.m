...
for idx2 = 1:length(snr)
    ...
    mse_sum_wosa  = zeros(N,N);
    bias_sum_wosa = zeros(N,N);
    mean_sum_wosa = zeros(N,N);    
    vari_sum_wosa = zeros(N,N);
    mse_sum_wosa2  = zeros(N,N);
    bias_sum_wosa2 = zeros(N,N);
    mean_sum_wosa2 = zeros(N,N);    
    vari_sum_wosa2 = zeros(N,N);
    mse_sum_wosa3  = zeros(N,N);
    bias_sum_wosa3 = zeros(N,N);
    mean_sum_wosa3 = zeros(N,N);    
    vari_sum_wosa3 = zeros(N,N);

    mse_sum_mvdr  = zeros(N,N);
    bias_sum_mvdr = zeros(N,N);
    mean_sum_mvdr = zeros(N,N);    
    vari_sum_mvdr = zeros(N,N);
    mse_sum_mvdr2  = zeros(N,N);
    bias_sum_mvdr2 = zeros(N,N);
    mean_sum_mvdr2 = zeros(N,N);    
    vari_sum_mvdr2 = zeros(N,N);
    mse_sum_mvdr3  = zeros(N,N);
    bias_sum_mvdr3 = zeros(N,N);
    mean_sum_mvdr3 = zeros(N,N);    
    vari_sum_mvdr3 = zeros(N,N);

    mse_sum_nmvdr  = zeros(N,N);
    bias_sum_nmvdr = zeros(N,N);
    mean_sum_nmvdr = zeros(N,N);    
    vari_sum_nmvdr = zeros(N,N);
    mse_sum_nmvdr2  = zeros(N,N);
    bias_sum_nmvdr2 = zeros(N,N);
    mean_sum_nmvdr2 = zeros(N,N);    
    vari_sum_nmvdr2 = zeros(N,N);
    mse_sum_nmvdr3  = zeros(N,N);
    bias_sum_nmvdr3 = zeros(N,N);
    mean_sum_nmvdr3 = zeros(N,N);    
    vari_sum_nmvdr3 = zeros(N,N);
    
    mse_sum_apes  = zeros(N,N);
    bias_sum_apes = zeros(N,N);
    mean_sum_apes = zeros(N,N);    
    vari_sum_apes = zeros(N,N);
    mse_sum_apes2  = zeros(N,N);
    bias_sum_apes2 = zeros(N,N);
    mean_sum_apes2 = zeros(N,N);    
    vari_sum_apes2 = zeros(N,N);
    mse_sum_apes3  = zeros(N,N);
    bias_sum_apes3 = zeros(N,N);
    mean_sum_apes3 = zeros(N,N);    
    vari_sum_apes3 = zeros(N,N);
    ...
    mse(idx2,1)       = mean(mean(mse_sum_wosa./trials)); 
    average(idx2,1)   = mean(mean(mean_sum_wosa)); 
    bias(idx2,1)      = mean(mean(mean_sum_wosa-h)); 
    variance(idx2,1)  = mean(mean(vari_sum_wosa)) + var(var(mean_sum_wosa));

    mse(idx2,2)       = mean(mean(mse_sum_wosa2./trials)); 
    average(idx2,2)   = mean(mean(mean_sum_wosa2)); 
    bias(idx2,2)      = mean(mean(mean_sum_wosa2-h)); 
    variance(idx2,2)  = mean(mean(vari_sum_wosa2)) + var(var(mean_sum_wosa2));

    mse(idx2,3)       = mean(mean(mse_sum_wosa3./trials)); 
    average(idx2,3)   = mean(mean(mean_sum_wosa3)); 
    bias(idx2,3)      = mean(mean(mean_sum_wosa3-h)); 
    variance(idx2,3)  = mean(mean(vari_sum_wosa3)) + var(var(mean_sum_wosa3));

    mse(idx2,4)       = mean(mean(mse_sum_mvdr./trials)); 
    average(idx2,4)   = mean(mean(mean_sum_mvdr)); 
    bias(idx2,4)      = mean(mean(mean_sum_mvdr-h)); 
    variance(idx2,4)  = mean(mean(vari_sum_mvdr)) + var(var(mean_sum_mvdr));

    mse(idx2,5)       = mean(mean(mse_sum_mvdr2./trials)); 
    average(idx2,5)   = mean(mean(mean_sum_mvdr2)); 
    bias(idx2,5)      = mean(mean(mean_sum_mvdr2-h)); 
    variance(idx2,5)  = mean(mean(vari_sum_mvdr2)) + var(var(mean_sum_mvdr2));
    
    mse(idx2,6)       = mean(mean(mse_sum_mvdr3./trials)); 
    average(idx2,6)   = mean(mean(mean_sum_mvdr3)); 
    bias(idx2,6)      = mean(mean(mean_sum_mvdr3-h)); 
    variance(idx2,6)  = mean(mean(vari_sum_mvdr3)) + var(var(mean_sum_mvdr3));
    
    mse(idx2,7)       = mean(mean(mse_sum_nmvdr./trials)); 
    average(idx2,7)   = mean(mean(mean_sum_nmvdr)); 
    bias(idx2,7)      = mean(mean(mean_sum_nmvdr-h)); 
    variance(idx2,7)  = mean(mean(vari_sum_nmvdr)) + var(var(mean_sum_nmvdr));
 
    mse(idx2,8)       = mean(mean(mse_sum_nmvdr2./trials)); 
    average(idx2,8)   = mean(mean(mean_sum_nmvdr2)); 
    bias(idx2,8)      = mean(mean(mean_sum_nmvdr2-h)); 
    variance(idx2,8)  = mean(mean(vari_sum_nmvdr2)) + var(var(mean_sum_nmvdr2));

    mse(idx2,9)       = mean(mean(mse_sum_nmvdr3./trials)); 
    average(idx2,9)   = mean(mean(mean_sum_nmvdr3)); 
    bias(idx2,9)      = mean(mean(mean_sum_nmvdr3-h)); 
    variance(idx2,9)  = mean(mean(vari_sum_nmvdr3)) + var(var(mean_sum_nmvdr3));

    mse(idx2,10)       = mean(mean(mse_sum_apes./trials)); 
    average(idx2,10)   = mean(mean(mean_sum_apes)); 
    bias(idx2,10)      = mean(mean(mean_sum_apes-h)); 
    variance(idx2,10)  = mean(mean(vari_sum_apes)) + var(var(mean_sum_apes));

    mse(idx2,11)       = mean(mean(mse_sum_apes2./trials)); 
    average(idx2,11)   = mean(mean(mean_sum_apes2)); 
    bias(idx2,11)      = mean(mean(mean_sum_apes2-h)); 
    variance(idx2,11)  = mean(mean(vari_sum_apes2)) + var(var(mean_sum_apes2));

    mse(idx2,12)       = mean(mean(mse_sum_apes3./trials)); 
    average(idx2,12)   = mean(mean(mean_sum_apes3)); 
    bias(idx2,12)      = mean(mean(mean_sum_apes3-h)); 
    variance(idx2,12)  = mean(mean(vari_sum_apes3)) + var(var(mean_sum_apes3));
end

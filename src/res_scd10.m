...
fc      = [1.0 1.25]';  % Frequencies of each sinusoid
...
pres = zeros(length(snr),4);   % probability of resolution
...
for idx2 = 1:length(snr)
    ...
    pres(idx2,4)  = success_apes/trials;
end
...
legend('WOSA N/4 length', 'MVDR N/4 length', 'NMVDR N/8 length', 'APES N/4 length');
...
print -deps -painters  img/scd_res10
...
fh = figure(2);
set(fh, 'color', 'white'); % sets the color to white
fig1 = plot(snr,pres(:,1),'o-',snr,pres(:,2),'^-',snr,pres(:,3));
set(fig1, 'LineWidth', 1.5, 'MarkerSize', 8.0);
legend('WOSA N/4 length', 'MVDR N/4 length', 'NMVDR N/8 length');
title('Probability of resolution of SCD Estimators','FontSize',16,'FontWeight', 'bold');
ylabel('P(\Gamma > 0)','FontSize',16,'FontWeight', 'bold');
xlabel('Signal to Noise Ratio (dB)','FontSize',16,'FontWeight', 'bold');
grid on;
set(gca, 'Box', 'off','TickDir', 'out', 'FontSize',16 ); % here gca means get current axis
print -deps -painters  img/scd_res09

...
%   Description: 
%        Obtain the probability of resolution of WOSA SCD estimators for different SNR
...
fc      = [0.5 2.0]';       % Frequencies of each sinusoid
...
for idx = 1:length(M)   % for each filter length
    for idx2 = 1:length(snr)
        ...
        for trial=[1:trials]    % Repeat experiment trials times
            ...
            % SCD Estimation
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [S1 f1] = scd2(yn,M(idx),'WOSA',fs,2*fc(1));
            [S2 f2] = scd2(yn,M(idx),'WOSA',fs,2*fc(2));
            [S3 f3] = scd2(yn,M(idx),'WOSA',fs,fc(1)+fc(2));
...
print -deps -painters  img/scd_res01

...
pres = zeros(length(df),12);   % probability of resolution
...
for idx = 1:length(df)   % for each filter length
        ...
        for trial=[1:trials]    % Repeat experiment trials times
            ...
            [S1 f1] = scd2(yn,N/2,'WOSA',fs,2*freqs(1));
            [S2 f2] = scd2(yn,N/2,'WOSA',fs,2*freqs(2));
            [S3 f3] = scd2(yn,N/2,'WOSA',fs,freqs(1)+freqs(2));
            ...
            if ((1/2)*(abs(S2(k2))+abs(S1(k1))) - abs(S3(k3))) > 0
                success_wosa3 = success_wosa3 + 1;
            end

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % SCD Estimation MVDR 
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [S1 f1] = scd2(yn,N/2,'MVDR',fs,2*freqs(1));
            [S2 f2] = scd2(yn,N/2,'MVDR',fs,2*freqs(2));
            [S3 f3] = scd2(yn,N/2,'MVDR',fs,freqs(1)+freqs(2));
            ...
            if ((1/2)*(abs(S2(k2))+abs(S1(k1))) - abs(S3(k3))) > 0
                success_mvdr3 = success_mvdr3 + 1;
            end

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % SCD Estimation MVDR 
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [S1 f1] = scd2(yn,N/2,'NMVDR',fs,2*freqs(1));
            [S2 f2] = scd2(yn,N/2,'NMVDR',fs,2*freqs(2));
            [S3 f3] = scd2(yn,N/2,'NMVDR',fs,freqs(1)+freqs(2));
            ...
            if ((1/2)*(abs(S2(k2))+abs(S1(k1))) - abs(S3(k3))) > 0
                success_nmvdr3 = success_nmvdr3 + 1;
            end

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % SCD Estimation MVDR 
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [S1 f1] = scd2(yn,N/2,'APES',fs,2*freqs(1));
            [S2 f2] = scd2(yn,N/2,'APES',fs,2*freqs(2));
            [S3 f3] = scd2(yn,N/2,'APES',fs,freqs(1)+freqs(2));
            ...
            if ((1/2)*(abs(S2(k2))+abs(S1(k1))) - abs(S3(k3))) > 0
                success_apes3 = success_apes3 + 1;
            end
        end
    
        pres(idx,1)   = success_wosa/trials;
        pres(idx,2)   = success_wosa2/trials;
        pres(idx,3)   = success_wosa3/trials;
        pres(idx,4)   = success_mvdr/trials;
        pres(idx,5)   = success_mvdr2/trials;
        pres(idx,6)   = success_mvdr3/trials;
        pres(idx,7)   = success_nmvdr/trials;
        pres(idx,8)   = success_nmvdr2/trials;
        pres(idx,9)   = success_nmvdr3/trials;
        pres(idx,10)  = success_apes/trials;  
        pres(idx,11)  = success_apes2/trials;
        pres(idx,12)  = success_apes3/trials;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pres = upsamp(pres);   % This is upsample only to add point to the graph, no new information is added to the data
df = upsamp(df);

fh = figure(1);
set(fh, 'color', 'white'); % sets the color to white
fig1 =  plot(df,pres(:,1),'o-',df,pres(:,2),'o--',df,pres(:,3),'o-.',df,pres(:,4),'^-',df,pres(:,5),'^--',df,pres(:,6),'^-.',df,pres(:,7),'s-',df,pres(:,8),'s--',df,pres(:,9),'s-.',df,pres(:,10),'h-',df,pres(:,11),'h--',df,pres(:,12),'h-.');
...
title('Probability of resolution of SCD Estimators (20dB SNR)','FontSize',16,'FontWeight', 'bold');
...
print -deps -painters  img/scd_res27


fh = figure(2);
set(fh, 'color', 'white'); % sets the color to white
set(0,'DefaultAxesLineStyleOrder','-|--|:|-.')
fig1 = plot(df,pres(:,1),'o-',df,pres(:,2),'o--',df,pres(:,3),'o-.',df,pres(:,4),'^-',df,pres(:,5),'^--',df,pres(:,6),'^-.',df,pres(:,7),'s-',df,pres(:,8),'s--',df,pres(:,9),'s-.');
set(fig1, 'LineWidth', 1.5, 'MarkerSize', 8.0);
...
grid on;
set(gca, 'Box', 'off','TickDir', 'out', 'FontSize',16 ); % here gca means get current axis
print -deps -painters  img/scd_res28

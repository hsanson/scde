...
%
%   Description:
%       Estimates the cross PSD of two signals using the Capon (MLM) estimator.
%       This implementation obtains the PSD by iterating over the range of
%       frequencies of interes.
%
%   Notes:
%        - Tested with Matlab 7.0.0.19901 (R14) and Octave 2.9.9

function [C,k]=capon(x1,x2,M,METHOD);
...
for l = 1:length(k)
   aM = exp(j*2*pi*k(l)*m);
   C(l) = M * (aM' * Ri11_R12_Ri22 * aM) / (aM' * Ri11 * aM * aM' * Ri22 * aM);
end
